...
const Footer = () => {
  return (
    ...
    >
      <div className='flex justify-center items-center md:items-start md:flex-row flex-col gap-[5rem] w-full'>
        <div className='flex flex-col justify-start gap-[2rem] mr-[2.5rem] flex-1'>
          <Image
            ...
            height={72}
          />
          <p className='font-normal text-dimWhite text-[2.2rem] leading-[4rem] mt-[1rem] max-w-[310px]'>
            為你提供聆聽超過 9,000
        ...
        <div className='flex justify-between flex-wrap gap-[4rem] flex-[1.5] w-full'>
          {footerLinks.map((footerLink) => (
            <div
              className='flex flex-col ss:my-0 my-[1rem] min-w-[150px]'
              key={footerLink.title}
            >
              <h4 className='font-medium text-[2.3rem] text-white mb-[2.5rem]'>
                {footerLink.title}
              </h4>
              <ul className='mt-[1rem] list-none'>
        ...
        {socialMedia.map((social, index) => (
          <Image
            ...
            className={`object-contain cursor-pointer hover:scale-110 transition-all duration-200 ease-linear ${
              index !== socialMedia.length - 1 ? 'mr-[1.5rem]' : 'mr-0'
            }`}
          />
        ))}
      </div>

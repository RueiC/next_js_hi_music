...
const RegistrationForm = ({ providers, setToggleRegistrationForm }) => {
    ...
    <>
      <div className='hidden md:flex items-center justify-center md:fixed top-0 left-0 w-full h-[100vh] z-50 bg-black/75'>
        <div className='relative flex items-center justify-between bg-black-gradient w-[121rem] h-[76rem] rounded-[3rem]'>
          ...
          <AiFillCloseCircle
            className='absolute top-10 right-10 text-white text-[3.5rem] opacity-80 hover:opacity-100 hover:scale-110 transition-all duration-300 ease-in-out cursor-pointer'
            onClick={() => setToggleRegistrationForm(false)}
          />
        </div>
      </div>
